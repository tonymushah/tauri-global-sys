...
use tauri_global_sys::{
    ...
    fs::{rename_file, BaseDirectory, FsOptions},
};
...
#[component]
pub fn RenameFile() -> impl IntoView {
        ...
        Action::<_, anyhow::Result<ActionTypeKind>>::new_local(move |input: &ActionType| {
            ...
            async move {
                match input {
                    ...
                    ActionType::RenameMove(form_data) => {
                        let (src, dest) = (
                            source
                                .get_untracked()
                        ...
                        log::debug!("{:#?}", options);
                        rename_file(&src, &dest, options)
                            .await
                            .map_err(|e| anyhow::Error::msg(e.to_string()))?;
                        Ok(ActionTypeKind::RenameMove)
                    }
                }
            }
        });
    ...
    view! {
        {move || {
            match action_data.read().as_ref().filter(|_| is_loading.read() == false) {
                ...
                Some(Ok(ActionTypeKind::RenameMove)) => {
                    view! { <p style:color="green">"File renamed/moved!"</p> }.into_any()
                }
                _ => None::<()>.into_any(),
            }
        }}
        <form on:submit=move |ev| {
            ...
            action.dispatch_local(ActionType::RenameMove(form_data));
        }>
            <p on:click=move |_| {
            ...
            />
            <button type="submit">"Rename/move ;)))"</button>
        </form>
    }
}
