use super::styles;
use leptos::prelude::*;
use tauri_global_sys::dialog::{message, MessageDialogOptions};
    ...
    view! {
        {move || {
            match action_value.read().as_ref().filter(|_| action_status.read() == false) {
                ...
                Some(Ok(_)) => view! { <p style:color="green">"Message displayed"</p> }.into_any(),
                _ => None::<()>.into_any(),
            }
        }}
        <form
